...
# Output file
OUTPUT="pal-cav-frame-ranges.txt"
> "$OUTPUT"  # Clear the file

echo "========================================"
echo "PAL CAV Frame Range Probe Script"
echo "========================================"
echo ""
...
while IFS= read -r filepath; do
    ...
    ld-decode --PAL --start $safe_start --length 1 "$filepath_clean" /tmp/pal_cav_decode_$$ 2>&1 > /tmp/ld_output_$$_1.txt
    ...
    while true; do
        ...
        ld-decode --PAL --start $probe_pos --length 1 "$filepath_clean" /tmp/pal_cav_decode_$$ 2>&1 > /tmp/ld_output_$$_2.txt
